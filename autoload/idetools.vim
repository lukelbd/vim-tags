...
" | tr -s ' ' | sed '".'s$/\(.\{0,60\}\).*/;"$/\1.../$'."' "
function! idetools#ctags_display()
  exe "!clear; " . s:ctagcmd()
...
function! idetools#ctagmenu(ctaglist) " returns nicely formatted string
...
function! idetools#ctagselect(ctag)
...
function! idetools#change_repeat()
...
function! idetools#set_search(map)
  let motion = ''
  if a:map =~ '!'
    let string = getline('.')
    if len(string) == 0
...
function! idetools#change_next(map)
...
" Delete next match
function! idetools#delete_next(map)
  let action = idetools#set_search(a:map)
  if action == ''
    return
  endif
...
function! idetools#delete_all(map)
  let action = idetools#set_search(a:map)
  if action == ''
    return
  endif
